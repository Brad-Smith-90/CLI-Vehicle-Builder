...
class Cli {
  // The vehicles array now supports Truck, Car, and Motorbike objects
  private vehicles: (Car | Motorbike | Truck)[];
  selectedVehicleVin: string | undefined;
  exit: boolean = false;

  // The constructor accepts an array of vehicles, which could be a mix of Cars, Motorbikes, and Trucks
      ...
      .prompt([
        {
          ...
          choices: ['Car', 'Motorbike', 'Truck'], // All vehicle options are included
        },
      ])
      .then((answers) => {
  ...
  // This method lets the user choose a vehicle to tow with their truck
findVehicleToTow(truck: Truck): Promise<void> {
  return inquirer
    .prompt([
      {
        ...
        choices: this.vehicles
          .filter(vehicle => vehicle.vin !== truck.vin) // Don't let the truck tow itself
          .map(vehicle => {
            return {
              name: `${vehicle.vin} -- ${vehicle.make} ${vehicle.model}`,
              value: vehicle,
    ...
    .then((answers) => {
      ...
      // Check if the selected vehicle is light enough to be towed by the truck
      if (vehicleToTow.weight <= truck.towingCapacity) {
        ...
        console.log(`The vehicle is too heavy to be towed by this truck.`);
      }

      // After towing, go back to the action choices
      this.performActions();
    });
}
    ...
    .prompt([
      {
        ...
        // Add all relevant actions, including the new ones for Truck and Motorbike
        choices: [
          'Print details',
          'Start vehicle',
          'Accelerate 5 MPH',
          ...
          'Turn left',
          'Reverse',
          'Tow a vehicle',       // Option to tow for Truck
          'Do a wheelie',        // Option for Motorbike
      ...
      switch (answers.action) {
        ...
        case 'Tow a vehicle':
          if (selectedVehicle instanceof Truck) {
            ...
            return;  // Important to return here to avoid calling performActions immediately again
          }
          break;
        case 'Do a wheelie':
        ...
        case 'Select or create another vehicle':
          this.startCli();
          return;  // Return here to avoid double prompt
        case 'Exit':
          this.exit = true;
          break;
      }
